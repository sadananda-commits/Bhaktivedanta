...
//
// Issue 1 addition: each row now also records TimeTakenSeconds — how long
// the student spent on that specific question before answering it. Add a
// "TimeTakenSeconds" column to the StudentProgress tab (Code.gs's
// recordProgress handler should append it after the existing columns; it's
// backward-compatible since older rows simply have it blank).
...
export default async function handler(req, res) {
  ...
  const {
    ...
    questionNumber, answerGiven, correctAnswer, status, timeTakenSeconds,
  } = req.body || {};
  ...
  const payload = {
    progress: {
      ...
      Status:           status || '',
      TimeTakenSeconds: timeTakenSeconds !== undefined && timeTakenSeconds !== null ? String(timeTakenSeconds) : '',
