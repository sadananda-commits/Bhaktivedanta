...
export default function CallManager({ profile, callRequest, onCallRequestHandled }) {
  ...
  // Only one <video> element may ever hold remoteVideoRef/localVideoRef at
  // a time — a ref can't be split across two simultaneously-mounted DOM
  // nodes. This was the actual video bug: there used to be a second,
  // always-hidden pair of <video> tags ALSO using these same refs, so
  // whichever element React attached last silently "won" the ref, which in
  // practice was the hidden pair — audio still played (a hidden <video>
  // still outputs audio just fine), but the visible video-call UI never
  // received the stream. Now there is exactly one <video> per ref, ever,
  // and only its style changes between "hidden" and "full video call UI".
  const isVideoCall = callInfo?.callType === 'video';
  const showVideoUI = isVideoCall && (phase === 'connecting' || phase === 'connected');

  // ─── Render ──────────────────────────────────────────────────────────────
  return (
    <>
      {/* Single remote/local video pair for the whole call's lifetime —
          position:fixed so they can lay directly on top of the page without
          needing to live inside a specific positioned parent container. */}
      <video
        ref={remoteVideoRef}
        autoPlay
        playsInline
        style={showVideoUI ? styles.remoteVideo : styles.hiddenMedia}
      />
      <video
        ref={localVideoRef}
        autoPlay
        playsInline
        muted
        style={showVideoUI ? styles.localVideo : styles.hiddenMedia}
      />
      ...
      {/* Video call chrome (black backdrop + info/end bar) — the actual
          <video> elements above are NOT inside this, they're independently
          position:fixed, so they never remount when this toggles. */}
      {showVideoUI && (
        <>
          <div style={styles.videoCallBackdrop} />
