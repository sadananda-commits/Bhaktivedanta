...
const GroupChat = forwardRef(function GroupChat({ profile, t, focusGroupId, classLevels, onStartCall }, ref) {
  ...
  // Fallback to polling if context isn't available. Only runs while the
  // widget is actually open — closed AND minimized both stop it, since a
  // tucked-away or hidden widget has no need to know who's online right now.
  useEffect(() => {
    if (!profile?.id || widgetState !== 'open') return;
  ...
  // ── Load this student's groups — lazily, only while the widget is open.
  // A closed or minimized widget has no reason to be hitting this endpoint
  // at all; this fires once when it's first opened, and again each time
  // it's reopened after being closed (cheap single call, not a loop) so the
  // list stays fresh without any background activity while it's tucked
  // away. ────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!profile?.id || widgetState !== 'open') return;
    if (groups.length === 0) setGroupsLoading(true); // first-ever open: show the loading state
    setGroupsError('');
    fetch(`/api/student/chat-groups?studentId=${encodeURIComponent(profile.id)}`)
      .then(r => r.json())
      .then(data => {
        if (data.error) { setGroupsError(data.error); return; }
  ...
  // ── Full history load whenever the active group changes ──────────────────
  // activeGroupId only ever gets set while the widget is open (see the
  // groups-loading effect above), so this naturally doesn't fire while
  // closed. It also deliberately does NOT depend on widgetState — minimizing
  // and reopening the SAME chat shouldn't wipe and re-fetch everything,
  // just pause/resume the poll below.
  useEffect(() => {
  ...
  // ── Recurring message poll — the actual "ongoing chat" background
  // activity. This is the piece that must start/stop with the widget:
  // it only runs while widgetState === 'open', and the cleanup function
  // clears the interval the instant the widget is minimized or closed —
  // no more requests hit the backend until it's reopened. ─────────────────
  useEffect(() => {
    if (!activeGroupId || widgetState !== 'open') return;
    ...
    // Catch up immediately on open/reopen — otherwise a chat reopened after
    // being minimized would sit stale for up to POLL_MS before its first tick.
    if (lastTimestampRef.current) poll();
    pollRef.current = setInterval(poll, POLL_MS);

    return () => { clearInterval(pollRef.current); pollRef.current = null; };
  }, [activeGroupId, fetchMessages, widgetState]);
  ...
  if (groupsLoading) {
    return (
      <div className="gcw-dock">
        <div className="gc-card gc-empty" style={{ position: 'relative' }}>
          <button
            onClick={() => setWidgetState('closed')}
            aria-label="Close chat"
            style={{
              position: 'absolute', top: 8, right: 8, border: 'none', background: 'none',
              color: 'rgba(255,255,255,.6)', cursor: 'pointer', fontSize: 14, padding: 4, lineHeight: 1,
            }}
          >
            <i className="fa-solid fa-xmark" />
          </button>
    ...
    <div className="gc-card">
      <div className="gc-header">
        <div className="gc-title-row">
          <div className="gc-title-id">
            ...
            {hasGroups ? (
              groups.length > 1 ? (
                ...
                <span className="gc-title-name">{activeGroup?.groupName}</span>
              )
            ) : (
              <span className="gc-title-name">Chat</span>
            )}
            {otherMember && (
              <span
          ...
          {/* Window controls live in their own flex-shrink:0 group, in a row
              by themselves apart from anything variable-width (call buttons,
              New chat / New group) — that's what keeps them from ever being
              pushed past the card's edge and clipped by overflow:hidden. */}
          <div className="gc-window-controls">
            <button className="gc-window-btn" onClick={() => setWidgetState('minimized')} title="Shrink" aria-label="Minimize chat">
              <i className="fa-solid fa-minus" />
            </button>
            <button className="gc-window-btn gc-window-btn-close" onClick={() => setWidgetState('closed')} title="Close" aria-label="Close chat">
              <i className="fa-solid fa-xmark" />
            </button>
          </div>
        </div>

        {(otherMember || canStartDm) && (
          <div className="gc-action-row">
            ...
            {canStartDm && (
              <div className="gc-newchat-group">
      ...
      <style jsx>{`
        ...
        .gc-title-id i.fa-user, .gc-title-id i.fa-comments { flex-shrink: 0; }
        .gc-title-name {
          overflow: hidden;
        ...
        .gc-newchat-group { display: flex; align-items: center; gap: 6px; margin-left: auto; flex-wrap: wrap; }
        .gc-newchat-btn {
        ...
        .gc-window-controls { display: flex; align-items: center; gap: 2px; margin-left: 8px; flex-shrink: 0; }
        .gc-window-btn {
        ...
        .gc-window-btn-close:hover { background: #FDEDED; color: #B91C1C; }
