...
function rowsToObjects(rows, tabName, fieldsOnly) {
  if (!rows.length) return [];
  const headers = rows[0].map(h => h.trim());
  // fieldsOnly (optional Set<string>): when provided, only these columns are
  // ever read out of each row — every other column (Question, Options,
  // Explanation, any oversized pasted-blob column, etc.) is skipped
  // entirely, including the truncation check below. This matters a lot for
  // "Learning Steps" in summary mode: without it, a request with no
  // classLevel filter (i.e. previewing "all classes") fully parses EVERY
  // column of EVERY row of EVERY class's question bank into memory before
  // summaryMode ever gets a chance to strip it back down — which is what
  // was blowing the Node heap. With fieldsOnly, that heavy data is never
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    headers.forEach((h, i) => {
      if (!h) return;
      if (fieldsOnly && !fieldsOnly.has(h)) return; // skip — never read, never truncated, never held
...
async function fetchTab(sheetId, tabName, overrideUrl, fieldsOnly) {
  ...
  try {
    ...
    const rows = rowsToObjects(parseCSV(text), tabName, fieldsOnly);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const fetchTabCached = (sheetId, tabName, fieldsOnly) => {
      const key = `${sheetId}::${tabName}::${fieldsOnly ? [...fieldsOnly].join(',') : ''}`;
      if (!tabFetchCache.has(key)) tabFetchCache.set(key, fetchTab(sheetId, tabName, undefined, fieldsOnly));
    ...
    // In summary mode, the response only ever keeps Module ID + Class per
    // step (see responseSteps below) — so when summary mode is active, only
    // ever read that one column out of "Learning Steps" in the first place.
    // This is what actually fixes the "preview another class" OOM crash:
    // previously summaryMode only trimmed the final response, by which
    // point every column of every row of every class's ENTIRE question bank
    // (an unfiltered fetch, since no classLevel is passed for a full
    // cross-class preview) had already been parsed and held in memory at
    // once — the huge pasted-blob columns included, up to MAX_CELL_LENGTH
    // each, multiplied across every row and every class simultaneously.
    const stepsFieldsOnly = (summaryMode && drillMode !== 'questions') ? new Set(['Module ID']) : undefined;

    const results = await Promise.all(
      sheetEntries.flatMap(entry => [
        fetchTabCached(entry.sheetId, 'Learning Modules'),
        needSteps ? fetchTabCached(entry.sheetId, 'Learning Steps', stepsFieldsOnly) : Promise.resolve([]),
      ])
    );
    ...
    // In summary mode, every column except Module ID was already skipped at
    // fetch time (see stepsFieldsOnly above) — this is what keeps a
    // content-heavy, all-classes preview from ever holding every row's full
    // column set (large pasted answer/image content included) in memory at
    // once, not just from blowing past JSON.stringify's limit at the end.
    // (Summary mode and the drill-down modes are orthogonal — summary is
    // still useful for a caller that wants ALL subjects' counts at once,
    // e.g. the Parent Portal builder, without switching to per-chapter
    // fetches.)
    // learningSteps rows are already reduced to just Module ID (+ Class,
    // stamped above) at the source when summaryMode is on — see
    // stepsFieldsOnly above — so no further reduction is needed here.
    const responseSteps = learningSteps;
