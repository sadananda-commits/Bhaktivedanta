...
import OnlineQuizManager from '../components/OnlineQuizManager'; 
...
function ParentPortalInner() {
  ...
  const [ptUsername, setPtUsername] = useState(''); // ? ADD  -- see note above on why this exists
  ...
  const [mainView, setMainView] = useState('student'); // 'student' | 'quizzes'
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (data.authenticated) {
        setPtUser(data);
        setPtUsername(String(fd.get('username') || '').trim());   // ? ADD
        setAuthed(true);
          ...
          {({ sbOpen, setSbOpen }) => (
            <div className="ptlayout">
              ...
              <aside className={`ptsb${sbOpen?' open':''}`}>
                ...
                {ptUser?.role === 'Teacher' && (
                  <button
                    ...
                    onClick={() => setMainView(v => v === 'quizzes' ? 'student' : 'quizzes')}
                  >
                    <div className="ptsb-student-av"><i className="fa-solid fa-gamepad" /></div>
                    <div style={{fontWeight:700,color:'inherit'}}>Online Quizzes</div>
                  </button>
                )}
              ...
              <main className="ptmain">
                ...
                {mainView==='student' && !dataLoading && !dataErr && !selectedStudent && (
                ...
                {mainView==='student' && !dataLoading && selectedStudent && (
                  <>
                    {/* Page header */}
                ...
                {mainView === 'quizzes' && (
                  <OnlineQuizManager hostEmail={ptUsername} />
                )}
              </main>
            </div>
          )}
