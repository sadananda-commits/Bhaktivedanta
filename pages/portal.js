...
// Maximally-distinct colors for the Dashboard's per-subject charts (Subject
// Progress + Performance Ratios). Deliberately NOT the sheet's own
// Color (Hex) per subject — those are teacher-set and often collide (e.g.
// every subject not explicitly listed in SUBJECT_COLORS falls back to the
// same default teal), which is exactly what made the two charts hard to
// read. Assigned by each subject's position in SUBJ, so the same subject
// always gets the same color across both charts, and cycles if there are
// ever more subjects than colors.
...
import { isRowActive, useSoundEngine, FireworksOverlay, StreakToast, useTestTimer, LearningModulePlayer } from '../components/QuizPlayer';
...
{ID:'onlinequiz', Label:'Online Quiz', 'Icon (FontAwesome solid)':'fa-gamepad', Active:true, Order:4.6},
...
function PortalInner({ initialProfile }) {
  ...
  // Raw StudentProgress rows (one per answered question, with Subject/Topic/
  // Timestamp) from progress-history.js. Previously only used once to rebuild
  // learnProgress on login and then discarded — kept around here so the
  // Dashboard's "last 3 days" card and per-subject accuracy can be computed
  // directly from real timestamped events instead of just cumulative counts.
  const [rawProgressRows, setRawProgressRows] = useState([]);
  ...
  // This now drives an actual server-side re-aggregation (via ?days=) rather
  // than a client-side filter of an all-time dataset — see the fetch effects
  // below. Both `leaderboard` and `weightedLeaderboard` reflect whatever
  // window is currently selected.
  const [lbDays, setLbDays] = useState(30); // default: last 30 days
  // "Today's Activity" board — always shows today, independent of the Period
  // pills above, so fetched as its own fixed days=1 request rather than
  // derived from whatever window `leaderboard` currently holds.
  const [dailyLeaderboard,        setDailyLeaderboard]        = useState({ overall:[], bySubject:{} });
  ...
  // Dashboard-only chart instances (items 2 & 3) — kept separate from
  // subjectRef/ratioRef above, which still back the Progress tab's own
  // "Completion by Subject" / "Overall ratios" cards (different metric,
  // different meaning) even though both tabs used to share one canvas id
  // and one dataset, which meant changing one silently changed the other.
  const dashSubjectRef = useRef(null);
  const dashRatioRef   = useRef(null);
  ...
  // Original config-driven ratio stats — still used by the Progress tab's
  // "Overall ratios" card, unchanged. The Dashboard's "Performance Ratios"
  // card (below) now uses real per-subject accuracy instead.
  const legacyRatioStats = cfg.dashStats.filter(s => isRowActive(s.Active) && s['Metric Key'] && s['Metric Key'].startsWith('ratio_'))
    .map(s => s['Metric Key']==='ratio_quiz_avg' && liveQuizAvg!==null ? { ...s, Value:String(liveQuizAvg) } : s);

  // Item 3: "Performance Ratios" — real per-subject accuracy (questions
  // correct ÷ questions attempted), not the generic config-driven ratio_*
  // rows. Only subjects the student has actually attempted something in
  // show up, so a fresh student doesn't see a donut full of 0%s.
  const ratioStats = SUBJ.filter(s => s.QuestionsAttempted > 0).map(s => ({
    Label: s.Subject,
    Value: String(Math.round((s.QuestionsCorrect / s.QuestionsAttempted) * 100)),
    'Color (Hex)': s['Color (Hex)'],
  }));

  // Item 1: "last 3 days" activity — replaces the "Next Classes" card.
  // Computed straight from rawProgressRows (real StudentProgress rows, one
  // per answered question, each with Subject/Topic/Timestamp) rather than
  // the cumulative learnProgress counters, since this needs to know exactly
  // *when* each question was answered, not just running totals.
  const last3Days = (() => {
    const cutoff = Date.now() - 3 * 24 * 60 * 60 * 1000;
    const recent = rawProgressRows.filter(r => {
      const ts = r.Timestamp ? new Date(r.Timestamp).getTime() : NaN;
      return !isNaN(ts) && ts >= cutoff;
    });
    const subjects = new Set(recent.map(r => r.Subject).filter(Boolean));
    const chapters = new Set(recent.map(r => `${r.Subject}|${r.Topic}`).filter(k => k.split('|')[1]));
    return {
      questionsAttempted: recent.length,
  ...
  const destroyCharts = useCallback(() => {
    [subjectRef, attendRef, ratioRef, dashSubjectRef, dashRatioRef].forEach(r => { if (r.current) { r.current.destroy(); r.current = null; } });
  }, []);
  ...
  const initCharts = useCallback(() => {
    ...
    const dsEl = document.getElementById('dashSubjectChart');
    const drEl = document.getElementById('dashRatioChart');

    // Progress tab's own charts — unchanged: topic-completion % and the
    // original config-driven ratio stats, matching that tab's own titles
    // ("Completion by Subject" / "Overall ratios").
    ...
    if (rEl && legacyRatioStats.length) ratioRef.current = new window.Chart(rEl, {
      type: 'doughnut',
      data: { labels: legacyRatioStats.map(r => r.Label.replace(/\s*\([^)]*\)\s*$/, '')), datasets: [{ data: legacyRatioStats.map(r => Number(r.Value)||0), backgroundColor:['rgba(0,198,167,.85)','rgba(168,85,247,.85)','rgba(245,166,35,.85)'], borderWidth:0, hoverOffset:6 }] },
    ...
    // Stable subject → distinct color mapping, built fresh from SUBJ's own
    // order each time (cheap — SUBJ is already a short list) so the same
    // subject always lands on the same color in both charts below.
    const subjectColorMap = {};
    SUBJ.forEach((s, i) => { subjectColorMap[s.Subject] = DISTINCT_CHART_COLORS[i % DISTINCT_CHART_COLORS.length]; });

    // Dashboard's own charts (items 2 & 3) — "Subject Progress" now plots
    // questions attempted per subject (dynamic scale, not a 0-100 percent
    // axis), and "Performance Ratios" shows real per-subject accuracy.
    // Both use subjectColorMap (above) instead of each subject's own
    // Color (Hex) from the sheet, since those often collide.
    if (dsEl && SUBJ.length) dashSubjectRef.current = new window.Chart(dsEl, {
      type: 'bar',
      data: { labels: SUBJ.map(s => s.Subject), datasets: [{ label:'Questions Attempted', data: SUBJ.map(s => s.QuestionsAttempted), backgroundColor: SUBJ.map(s => subjectColorMap[s.Subject]), borderRadius: 8, borderWidth: 0 }] },
      options: { responsive:true, maintainAspectRatio:false, plugins:{legend:{display:false}}, scales:{ y:{beginAtZero:true,grid:{color:'rgba(255,255,255,.06)'},ticks:{color:'#94a3b8',font:{size:11},precision:0}}, x:{grid:{display:false},ticks:{color:'#94a3b8',font:{size:11}}} } },
    });

    if (drEl && ratioStats.length) dashRatioRef.current = new window.Chart(drEl, {
      type: 'doughnut',
      data: { labels: ratioStats.map(r => r.Label), datasets: [{ data: ratioStats.map(r => Number(r.Value)||0), backgroundColor: ratioStats.map(r => subjectColorMap[r.Label] || '#00c6a7'), borderWidth:0, hoverOffset:6 }] },
      options: { responsive:true, maintainAspectRatio:false, cutout:'68%', plugins:{ legend:{ position:'bottom', labels:{ color:'#94a3b8', font:{size:12}, boxWidth:12, padding:14 } } } },
    });
  }, [cfgReady, SUBJ, AMON, ratioStats, legacyRatioStats, destroyCharts]);
  ...
  // ── Public guest quiz link ───────────────────────────────────────────────
  // Used ONLY by the "Assignments" (open Question Bank) share button — never
  // by "Assignments for you", since those are personal, range-restricted
  // rows tied to one specific student and shouldn't be handed out publicly.
  // Unlike copyShareLink above (which copies the internal /portal?moduleId=
  // deep link, useless to a logged-out visitor), this builds a link to the
  // new public pages/quiz.js guest page. It carries subject + classLevel
  // alongside moduleId because a guest has no profile.classLevel of their
  // own for the API to filter by — see pages/quiz.js for how it's consumed.
  const copyPublicQuizLink = useCallback(() => {
    const qs = new URLSearchParams({
      moduleId:   activeModuleId || '',
      subject:    activeAssignmentSubject || '',
      classLevel: profile.classLevel || '',
    });
    const url = `${window.location.origin}/quiz?${qs.toString()}`;
    const flash = () => { setShareCopied(true); setTimeout(() => setShareCopied(false), 1800); };
    if (navigator.clipboard?.writeText) {
      navigator.clipboard.writeText(url).then(flash).catch(() => window.prompt(t('p_copy_link_prompt') || 'Copy this link:', url));
    } else {
  ...
  }, [t, activeModuleId, activeAssignmentSubject, profile.classLevel]);
  ...
  // Re-fetches whenever the selected Period pill (lbDays) changes, since the
  // server now re-aggregates points per window rather than the client
  // filtering an all-time dataset.
  useEffect(() => {
    if (tab !== 'leaderboard' || !authed) return;
    setLeaderboardLoading(true);
    fetch(`/api/student/leaderboard?days=${lbDays}`)
      .then(r => r.json())
  ...
  // tab is opened, and re-fetched on Period changes too. Kept as a separate
  // request/cache from the regular leaderboard since it's a different
  // aggregate (assignment-scoped, accuracy-ranked, min-50-attempts gated).
  useEffect(() => {
    if (tab !== 'leaderboard' || !authed) return;
    setWeightedLeaderboardLoading(true);
    fetch(`/api/student/weighted-leaderboard?days=${lbDays}`)
      .then(r => r.json())
  ...
  // "Today's Activity" board — fixed to days=1 regardless of the Period
  // pills, fetched once per tab-open (not on every lbDays change).
  useEffect(() => {
    if (tab !== 'leaderboard' || !authed) return;
    setDailyLeaderboardLoading(true);
    fetch('/api/student/leaderboard?days=1')
      .then(r => r.json())
      .then(data => setDailyLeaderboard({ overall: data.overall||[], bySubject: data.bySubject||{} }))
      .catch(() => {})
      .finally(() => setDailyLeaderboardLoading(false));
  }, [tab, authed]);
  ...
  // ── CSS ───────────────────────────────────────────────────────────────────────
  // Moved to lib/portalStyles.js (verbatim) so pages/quiz.js (the public guest
  // quiz page) can render identical styling without duplicating ~600 lines
  // of CSS. No styles changed — see that file for the actual rules.
      ...
      {!authed ? (
        ...
        <div className={`dash${activeModuleId ? ' quiz-mode' : ''}`}>
          ...
          <OnlineStudents 
            ...
            onGroupCall={() => setShowGroupCall(true)}
            profile={profile}
          ...
          <main className="main">
            ...
            {tab==='dashboard' && (<>
              ...
              <div className="content">
                ...
                <div className="cr">
                  <div className="card"><div className="card-t"><i className="fa-solid fa-chart-bar" /> {t('p_subject_progress')}</div><div className="cv"><canvas id="dashSubjectChart" /></div></div>
                  <div className="card"><div className="card-t"><i className="fa-solid fa-circle-half-stroke" /> {t('p_performance_ratios')}</div><div className="cv"><canvas id="dashRatioChart" /></div></div>
                </div>
                <div className="card">
                  <div className="card-t"><i className="fa-solid fa-bolt" /> Last 3 Days</div>
                  {last3Days.questionsAttempted === 0 ? (
                    <div style={{textAlign:'center',color:'var(--muted)',fontSize:'13px',padding:'14px 0'}}>No activity in the last 3 days — jump into the Question Bank to get started!</div>
                  ) : (
                    <div className="sr" style={{marginTop:'10px'}}>
                      <div className="sc">
                        <div className="sc-l">Subjects</div>
                        <div className="sc-v">{last3Days.subjectsCount}</div>
                        <div className="sc-s">touched in 3 days</div>
                      </div>
                      <div className="sc">
                        <div className="sc-l">Chapters</div>
                        <div className="sc-v">{last3Days.chaptersCount}</div>
                        <div className="sc-s">worked on</div>
                      </div>
                      <div className="sc">
                        <div className="sc-l">Questions</div>
                        <div className="sc-v">{last3Days.questionsAttempted}</div>
                        <div className="sc-s">attempted</div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {tab==='leaderboard' && (<>
              ...
              <div className="content">
                ...
                {/* ── TIME FILTER PILLS ───────────────────────────────────── */}
                {/* Rows are fetched already-aggregated for the selected window
                    (see the fetch effects above) — points, unique-question
                    counts and accuracy are all computed server-side per
                    ?days=, so nothing here re-filters or re-derives them. */}
                {(() => {
                  const opts = [
                    { days:1,   label:'Today'    },
                    { days:7,   label:'7 Days'   },
                  ...
                  const periodLabel = lbDays===0?'All time':lbDays===1?'Today':`Last ${lbDays} days`;

                  // ── TODAY'S ACTIVITY — always days=1, independent of the
                  // Period pills, from the dedicated dailyLeaderboard fetch.
                  const dailyRows = dailyLeaderboard.overall
                    .slice()
                  ...
                  return (
                    <>
                      ...
                      {/* ── TODAY'S ACTIVITY — POINTS SCORED ── */}
                      <div className="card" style={{marginBottom:'22px'}}>
                        <div className="card-t">
                          <i className="fa-solid fa-bolt" style={{color:'#f5a623'}} /> Today's Activity — Points Scored
                      ...
                      {/* ── OVERALL RANKING (server-filtered by Period) ── */}
                      <div className="card" style={{marginBottom:'22px'}}>
                        <div className="card-t">
                              ...
                              return (
                                <div key={row.studentId} className={`lb-row${isMe?' me':''}`}>
                                  ...
                                  <div className="lb-stats">
                                    <span className="lb-correct">{row.pointsScored} pts</span>
                      ...
                      {/* ── SUBJECT CHAMPIONS (server-filtered by Period) ── */}
                              ...
                              <div key={subject} className="card lb-subj-card">
                                ...
                                {champ ? (
                                  <>
                                    ...
                                    <div className="lb-champ-stats">{champ.pointsScored} pts · {champ.accuracy}% {t('p_accuracy')}</div>
                                    {rows.length>1 && (
                      ...
                      <div className="sec-divider">
                        ...
                        <span style={{marginLeft:'8px',fontWeight:400,fontSize:'10px',color:'var(--muted)'}}>
                          Assigned questions only · min. 50 unique attempted · {periodLabel} · ranked by accuracy
                        </span>
                      </div>
                      <div className="card" style={{marginBottom:'22px'}}>
                        <div className="card-t">
                        ...
                        {weightedLeaderboardLoading ? (
                          ...
                          <div style={{textAlign:'center',color:'var(--muted)',fontSize:'13px',padding:'20px 0'}}>
                            No students have answered at least 50 unique assigned questions in this period.
                          </div>
                        ) : (
                          <div className="lb-list">
                            {weightedLeaderboard.overall.slice(0,20).map((row,i)=>{
                              ...
                              return (
                                <div key={row.studentId} className={`lb-row${isMe?' me':''}`}>
                                  ...
                                  <div className="lb-name">{row.studentName}{isMe && <span className="lb-you">{t('p_you_suffix')}</span>}</div>
                                  <div className="lb-stats">
                                    <span className="lb-correct">{row.pointsScored} pts · {row.attempted} unique assigned</span>
                                    <span className="lb-acc">{row.accuracy}% {t('p_accuracy')}</span>
                                  </div>
                                </div>
                              );
                          ...
                          {Object.entries(weightedLeaderboard.bySubject).map(([subject, rows]) => {
                            ...
                            return (
                              <div key={subject} className="card lb-subj-card">
                                ...
                                <div className="lb-champ-name">{champ.studentName}</div>
                                <div className="lb-champ-stats">{champ.accuracy}% {t('p_accuracy')} · {champ.pointsScored} pts ({champ.attempted} unique assigned)</div>
                                {rows.length>1 && (
                                  <div className="lb-runner-ups">
                                    {rows.slice(1,4).map((r,ri)=>(
                                      <div key={r.studentId} className="lb-runner-row">
                          ...
                          <button
                            ...
                            onClick={()=>{
                              if (classFilter === 'all') {
                                ...
                                // Coming from "My Class" default — switch fully to this
                                // class's questions. Own-class subjects must turn off
                                // completely here, not stay merged in alongside it.
                                setClassFilter(new Set([cls]));
                              } else {
                                const next = new Set(classFilter);
                                if (isSet) {
                                  next.delete(cls);
                                  // Nothing left selected → back to "My Class" default
                                  if (next.size === 0) setClassFilter(null);
            ...
            {tab==='onlinequiz' && (
              <div className="card" style={{ maxWidth: 420, margin: '40px auto', textAlign: 'center' }}>
                <h2><i className="fa-solid fa-gamepad" /> Join an Online Quiz</h2>
                <p style={{ color: 'var(--muted)' }}>Enter the quiz code your teacher shared.</p>
                <input
                  value={quizCodeInput}
                  onChange={e => setQuizCodeInput(e.target.value.toUpperCase())}
                  placeholder="e.g. MATH7A"
                  style={{ width: '100%', padding: 12, borderRadius: 10, textAlign: 'center',
                ...
                <button
                  ...
                  onClick={() => router.push(`/quiz/${quizCodeInput.trim()}`)}
                >
                  Join Quiz
                </button>
              </div>
