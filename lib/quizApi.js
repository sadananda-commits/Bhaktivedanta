...
export const quizApi = {
  ...
  // Self-paced ("take it later") actions — for someone who missed the live
  // session. They join, get Question 1 immediately, and move themselves
  // through the quiz with their own per-question timer; nothing here is
  // broadcast to the host's live screen or other players. See Code.gs's
  // joinSoloQuiz_ / soloNextQuestion_ etc. for the server-side rules,
  // including what happens if the host ends the quiz mid-question.
  joinSoloQuiz: (quizId, name, age) => callQuiz('joinSoloQuiz', { quizId, name, age }),
  getSoloState: (quizId, participantId) => callQuiz('getSoloState', { quizId, participantId }),
  submitSoloAnswer: (quizId, participantId, qNum, selectedAnswer) =>
    callQuiz('submitSoloAnswer', { quizId, participantId, qNum, selectedAnswer }),
  soloNextQuestion: (quizId, participantId) => callQuiz('soloNextQuestion', { quizId, participantId }),
