...
// options.participantId: pass the CURRENT USER's own participant ID (only
// meaningful on the participant screen, never the host) to also watch for
// them specifically being removed — see the 'removed' event below.
// Returns an unsubscribe function — call it from a useEffect cleanup.
export function subscribeToQuiz(quizCode, handlers, options = {}) {
  const quizId = String(quizCode).toUpperCase();
  let prev = null;
  let sawFirstParticipantsSnapshot = false;
  const knownParticipantStatus = new Map(); // participantId -> last known status
  ...
  // 'participant-joined' / 'participant-removed' are both driven by the
  // participants subcollection, not the main doc, so each is caught the
  // instant its doc is written/changed, independent of whatever else
  // changed on the quiz doc that tick.
  const unsubParticipants = onSnapshot(
    ...
    (snap) => {
      snap.docChanges().forEach((change) => {
        ...
        const prevStatus = knownParticipantStatus.get(id);

        // Self-paced ("take it later") docs live in this same subcollection
        // (see pages/api/quiz.js's joinSoloQuiz) but deliberately don't
        // increment the live participantCount and shouldn't ring the "X
        // joined" sound/counter on the host's live screen — they're not
        // part of the live round at all. knownParticipantStatus still
        // tracks them (harmless, and keeps the map accurate if a solo doc
        // is ever inspected elsewhere), it just never reaches a handler.
        if (data.mode === 'solo') { knownParticipantStatus.set(id, data.status); return; }

        if (change.type === 'added') {
          knownParticipantStatus.set(id, data.status);
          ...
          handlers['participant-joined']?.({
            participantId: id,
            name: data.name,
            participantCount: snap.docs.filter(d => d.data().status !== 'left' && d.data().mode !== 'solo').length,
          });
          return;
        }

        if (change.type === 'modified') {
          const justRemoved = prevStatus !== 'left' && data.status === 'left';
          knownParticipantStatus.set(id, data.status);
          if (justRemoved) {
            handlers['participant-removed']?.({
              participantId: id,
              participantCount: snap.docs.filter(d => d.data().status !== 'left' && d.data().mode !== 'solo').length,
            });
          }
  ...
  // If a participantId was passed (the participant screen, never the host),
  // also watch that ONE doc specifically so being removed produces an
  // immediate, unmistakable "you were removed" screen — not just a silent
  // drop from everyone else's counts while they keep sitting in the quiz.
  let unsubSelf = () => {};
  if (options.participantId) {
    unsubSelf = onSnapshot(
      doc(db, 'quizzes', quizId, 'participants', options.participantId),
      (snap) => {
        if (snap.exists() && snap.data().status === 'left') {
          handlers['removed']?.();
        }
      },
      (err) => { console.error('quizFirestore: self-participant listener error', err.code || err); }
    );
  }

  return () => { unsubQuiz(); unsubParticipants(); unsubSelf(); };
}
